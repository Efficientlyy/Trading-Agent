// Enhanced Trades Display Implementation with Fallback for API Issues
console.log("Loading Enhanced Trades Display Implementation with Fallback");

document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM Content Loaded - Initializing enhanced trades display with fallback");
    ...
    function initEnhancedTrades() {
        console.log("Initializing enhanced trades display with fallback");
    ...
    // Fetch and render trades data with timeout and fallback
    async function fetchAndRenderTrades() {
        console.log("Fetching trades data with fallback");
        ...
        try {
            ...
            if (!data || data.length === 0) {
                console.warn("No trades data received, using fallback");
                createSampleTrades();
                return;
            }
            ...
            // If we've failed multiple times, use sample data
            if (apiFailureCount >= MAX_API_FAILURES) {
                console.warn(`API failed ${apiFailureCount} times, using sample data`);
                createSampleTrades();
                ...
                if (tradesContainer) {
                    ...
                    if (!document.querySelector('.api-error-message')) {
                        const errorMsg = document.createElement('div');
                        errorMsg.className = 'api-error-message';
                        errorMsg.textContent = 'Using sample trade data due to API connectivity issues';
                        tradesContainer.parentNode.insertBefore(errorMsg, tradesContainer);
                    }
                }
            }
        }
    ...
    function createSampleTrades() {
        ...
        const currentPrice = 104000 + Math.random() * 500;
        
        for (let i = 0; i < 20; i++) {
            // Create realistic price movements
            const priceChange = (Math.random() - 0.5) * 100;
            const price = currentPrice + priceChange * (i / 20);
