...
class PPOAgent:
    ...
    def update(self):
        ...
        if len(self.states) < self.update_timestep:
            return
        ...
        # Update metrics
        self.metrics["actor_losses"].append(np.mean(actor_losses))
        self.metrics["critic_losses"].append(np.mean(critic_losses))
        self.metrics["entropy"].append(np.mean(entropies))
        ...
        logger.info(f"Actor Loss: {np.mean(actor_losses):.4f}, Critic Loss: {np.mean(critic_losses):.4f}, Entropy: {np.mean(entropies):.4f}")
    ...
    def load(self, path):
        ...
        try:
            ...
            logger.info(f"Training step: {self.training_step}, Episodes: {self.episode_count}, Best reward: {self.best_reward:.4f}")
...
if __name__ == "__main__":
    ...
    # Create agent
    agent = PPOAgent(state_dim, action_dim, action_bounds)
