...
class TradingRLEnvironment:
    ...
    def _validate_action(self, action):
        ...
        if "continuous" in action:
            ...
            # Check if continuous action is empty
            if len(continuous_action) == 0:
                raise ValueError("Continuous action dictionary cannot be empty")
                
            # Check if at least one valid parameter exists
            valid_params = [param for param in continuous_action.keys() 
                           if param in self.config["action_space"]["continuous"]]
            if not valid_params:
                raise ValueError(f"No valid parameters found in continuous action. Valid parameters are: {list(self.config['action_space']['continuous'].keys())}")
        ...
        if "discrete" in action:
            ...
            # Check if discrete action is empty
            if len(discrete_action) == 0:
                raise ValueError("Discrete action dictionary cannot be empty")
                
            # Check if at least one valid parameter exists
            valid_params = [param for param in discrete_action.keys() 
                           if param in self.config["action_space"]["discrete"]]
            if not valid_params:
                raise ValueError(f"No valid parameters found in discrete action. Valid parameters are: {list(self.config['action_space']['discrete'].keys())}")
            ...
            for param, value in discrete_action.items():
                ...
                if value not in allowed_values:
                    ...
                    raise ValueError(f"Invalid value for {param}: {value}, must be one of {allowed_values}")
