#!/usr/bin/env python
"""
Enhanced Logging System for Trading-Agent (Fixed Version)
...
This version fixes the missing field errors by providing default values.
"""
...
class SafeContextFilter(logging.Filter):
    """Filter that adds contextual information to log records with safe defaults"""
    ...
    def filter(self, record):
        # Add context attributes to the record with safe defaults
        for key, default_value in self.defaults.items():
            if key in self.context:
                setattr(record, key, self.context[key])
            else:
...
class EnhancedLogger:
    ...
    def __init__(self, name="trading_agent"):
        ...
        self.name = name
        self.loggers = {}
        self.context_filter = SafeContextFilter()
    ...
    def clear_context(self, *keys):
        """Clear specific context attributes
        
        Args:
            *keys: Keys to clear from context
        """
        for key in keys:
            if key in self.context_filter.context:
                del self.context_filter.context[key]
    
    def reset_context(self):
        """Reset context to default values"""
        self.context_filter.context = {
            'system_id': self.context_filter.context.get('system_id', f"SYSTEM-{int(time.time())}"),
            'session_id': self.context_filter.context.get('session_id', f"SESSION-{int(time.time())}"),
            'version': self.context_filter.context.get('version', "1.0.0")
        }
...
if __name__ == "__main__":
    ...
    # Test direct logging without explicit context
    logger.system.info("This log should have default context values")
